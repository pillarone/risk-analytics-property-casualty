...
components {
    frequencyGenerator {
        parmDistribution[allPeriods] = RandomDistributionFactory.getDistribution(FrequencyDistributionType.CONSTANT, ["constant": 2d])
    }
    copula {
    ...
    fire {
        ...
        subClaimsGenerator {
            parmDistribution[allPeriods] = RandomDistributionFactory.getDistribution(DistributionType.NORMAL, ["mean": 10, "stDev": 2])
        }
    }
    hull {
        subSeverityExtractor {
    ...
    cxl {
        parmContractStrategy[allPeriods] = ReinsuranceContractStrategyFactory.getContractStrategy(ReinsuranceContractType.CXL,
