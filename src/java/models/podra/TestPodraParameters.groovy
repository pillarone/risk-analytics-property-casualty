...
components {
	reinsurance {
		subPropertyCxl {
			parmCoveredLines[0]=new ComboBoxTableMultiDimensionalParameter(GroovyUtils.toList([["property"]]),["Covered Lines"], LobMarker)
			parmCoveredPerils[0]=new ComboBoxTableMultiDimensionalParameter(GroovyUtils.toList([[""]]),["perils"], PerilMarker)
			parmCoveredReserves[0]=new ComboBoxTableMultiDimensionalParameter(GroovyUtils.toList([[""]]),["reserves"], IReserveMarker)
		...
		subMotorHullWxl {
			...
			parmCoveredLines[0]=new ComboBoxTableMultiDimensionalParameter(GroovyUtils.toList([["motor hull"]]),["Covered Lines"], LobMarker)
			parmInuringPriority[0]=1
			parmCommissionStrategy[0]=CommissionStrategyType.getStrategy(CommissionStrategyType.FIXEDCOMMISSION, ["commission":0.0,])
		}
