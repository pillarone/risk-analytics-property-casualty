	...
	mtpl {
		...
		subClaimsGenerator {
			subSingleClaimsGenerator {
				...
				subClaimsGenerator {
                    parmDistribution[allPeriods] = RandomDistributionFactory.getDistribution(ClaimSizeDistributionType.NORMAL, ["stDev":1.0, "mean":10.0])
					parmBase[0]=Exposure.ABSOLUTE
				}
			}
			subAttritionalClaimsGenerator {
                parmDistribution[allPeriods] = RandomDistributionFactory.getDistribution(ClaimSizeDistributionType.NORMAL, ["stDev":15, "mean":100])
				parmBase[0]=Exposure.ABSOLUTE
			}
		}
		subRiProgram {
