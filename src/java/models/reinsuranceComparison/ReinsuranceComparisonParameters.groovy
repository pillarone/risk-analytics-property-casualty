...
components {
		...
		subClaimsGenerator {
			subSingleClaimsGenerator {
				...
				subClaimsGenerator {
					...
					parmModification[0]=DistributionModifier.getStrategy(DistributionModifier.CENSORED, ["min":1000000.0, "max":100000000.0])
		...
		subClaimsGenerator {
			subSingleClaimsGenerator {
				...
				subClaimsGenerator {
					...
					parmModification[0]=DistributionModifier.getStrategy(DistributionModifier.TRUNCATED, ["min":500000.0, "max":20000000.0])
					parmBase[0]=Exposure.ABSOLUTE
				}
			}
	...
	property {
		...
		subClaimsGenerator {
			subSingleClaimsGenerator {
				...
				subClaimsGenerator {
					...
					parmModification[0]=DistributionModifier.getStrategy(DistributionModifier.TRUNCATED, ["min":70000.0, "max":1000000.0])
					parmBase[0]=Exposure.ABSOLUTE
				}
			}
            subAttritionalFrequencyGenerator{
                 parmDistribution[allPeriods] = DistributionType.getStrategy(DistributionType.POISSON,["lambda":1.0])
            }
        ...
        subRiProgramD {
			...
			subContract3 {
				parmContractStrategy[0] = ReinsuranceContractType.getStrategy(ReinsuranceContractType.STOPLOSS, ["premiumAllocation":PremiumAllocationType.getStrategy(PremiumAllocationType.PREMIUM_SHARES, [:]),"stopLossContractBase":StopLossContractBase.GNPI,"premium":0.1207,"attachmentPoint":3.3960398E7,"limit":1.01881194E8, "coveredByReinsurer": 1d])
				parmInuringPriority[0] = 0
			}
		}
    }
